...
/**
 * Collect the raw metadata for a single plugin setting before we infer any Nix types.
 *
 * The upstream plugins bounce between plain literals, Discord enum references, and helper
 * wrappers (e.g., `withPrivateSettings`). This helper grabs whatever is explicitly declared:
 * default literal, placeholder, hidden/ restart flags, and the `type` property when authors
 * spell it out. Anything dynamic gets resolved later by the inference pipeline.
 */
...
/**
 * Convert the scraped TS info into the final `PluginSetting` structure that the Nix generator
 * expects. We append the restart notice directly inside the description (mirroring how Vencord
 * renders it), drop empty enum arrays, and treat placeholders as `example` fields downstream.
 *
 * Hiding a setting in upstream code keeps it hidden here as well—this surfaces only the
 * public-facing options that would appear inside Vencord/Equicord’s settings UI.
 */
