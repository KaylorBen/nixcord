...
import type { ParsePluginsOptions } from './parser.js';
import { generateNixModule } from './nix/generator.js';
import { generateParseRulesModule } from './nix/parse-rules.js';
import { ParsedPluginsResultSchema, type ParsedPluginsResult } from './types.js';
import type { Logger } from './logger.js';

type SourceLabel = 'Vencord' | 'Equicord';

export interface GeneratePluginOptionsParams {
  vencordPath: string;
  equicordPath?: string;
...
export const runGeneratePluginOptions = async ({
  ...
  verbose = false,
  logger,
}: GeneratePluginOptionsParams): Promise<Result<GeneratePluginOptionsSummary, Error>> => {
  try {
