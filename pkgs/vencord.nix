  ...
  passthru.updateScript = writeShellApplication {
    ...
    text = ''
      ...
      if [ "$UPDATE_BOOL" = "true" ]; then
        ...
        revision=$(curl -s "https://api.github.com/repos/${finalAttrs.src.owner}/${finalAttrs.src.repo}/commits/main" | jq -r .sha)
        commit_date=$(curl -s "https://api.github.com/repos/${finalAttrs.src.owner}/${finalAttrs.src.repo}/commits/$revision" | jq -r '.commit.committer.date | split("T")[0]')
